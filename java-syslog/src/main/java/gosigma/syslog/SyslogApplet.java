...
class SyslogApplet extends Applet implements ActionListener {
	...
	public void init() {
		...
		try {
			this.syslog = new Syslog("SyslogTest", SyslogDefs.LOG_PERROR);
		} catch (SyslogException ex) {
			System.err.println("FATAL creating Syslog instance: '" + ex.getMessage() + "'");
		}
	...
	public void runTestOne() {
		...
		try {
			...
			this.syslog.syslog(addr, SyslogDefs.LOG_LOCAL0, SyslogDefs.LOG_ERR, "Test Syslog Message #1 (LOCAL0.ERR)");
		} catch (UnknownHostException ex) {
			String message = "error locating host named '" + host + "': " + ex.getMessage();
		} catch (SyslogException ex) {
			System.err.println("ERROR sending syslog message: '" + ex.getMessage() + "'");
		}
	}
	...
	public void runTestTwo() {
		...
		try {
			...
			this.syslog.syslog(addr, SyslogDefs.LOG_LOCAL0, SyslogDefs.LOG_ERR, "Test Syslog Message #2 (LOCAL0.ERR)");
		} catch (UnknownHostException ex) {
			String message = "error locating host named '" + host + "': " + ex.getMessage();
		} catch (SyslogException ex) {
			System.err.println("ERROR sending syslog message: '" + ex.getMessage() + "'");
	...
	public void runFloodTest() {
		String host = this.hostname.getText();
		try {
			InetAddress addr = InetAddress.getByName(host);

			for (int j = 0; j < 25; ++j) {
				this.syslog.syslog(addr, SyslogDefs.LOG_LOCAL0, SyslogDefs.LOG_ERR,
	...
	public void sendMessage() {
		...
		if (facInt == null || priInt == null) {
			System.err.println("ERROR " + (facInt == null ? "facility" : "level") + " is null! fac '" + facStr
					+ "' lvl '" + priStr + "'");
		} else {
			String host = this.hostname.getText();
			try {
				...
				this.syslog.syslog(addr, facInt.intValue(), priInt.intValue(), this.message.getText() + "\n");
			} catch (UnknownHostException ex) {
				String message = "error locating host named '" + host + "': " + ex.getMessage();
			} catch (SyslogException ex) {
	...
	establishContents()
		...
		enumer = this.priHash.keys();
		for ( ; enumer.hasMoreElements() ; )
			{
			String keyStr = null;

			try { keyStr = (String)enumer.nextElement(); }
	...
	public void constrain(Container container, Component component, int fill, int anchor, int gx, int gy, int gw,
